...
产出：清单 JSON、ZIP 布局、sha256 命令、登记表单要填的中文栏。登记来源二选一：上传 ZIP，或公开 HTTPS。免费条目直接用公开地址。收费条目上传 ZIP 或给出公开地址，由本站拉一次后保管，开发者不填令牌、不需要自己的仓库。
...
面板：**登记插件** / **登记模板**。包来源二选一：「上传压缩包」「公开地址」。收费包由本站保管，页面显示「本站保管」，不显示仓库地址。开发者不填令牌。按钮灰掉时，表单里有中文原因。
...
| 版本 | `version` | `1.0.0` |
| 包来源 | `packageSource` | `upload` 或 `public`。收费也可以用公开地址，本站会拉取一次 |
| 下载地址 | `downloadUrl` | 仅插件。上传后由本站保管；免费外链为 `https://` ZIP；收费公开地址由本站拉取后保管 |
| 模板地址 | `templateUrl` | 仅模板。规则同下载地址 |
| 校验码 (SHA256) | `sha256` | 免费公开地址用上一步输出。收费由本站自动计算 |
...
4. 上表每一栏的值
5. 一句话：免费用公开地址，提交时校验；收费上传 ZIP 或给公开地址，由本站保管，开发者不填令牌；登录框由宿主按 `stylePreset` 打开（与首页同一套颜色和圆角）；模板包内不要 `index.html` 或 `login.html`
...
| `需要重新上传 zip 或填写公开地址，才能继续收费出售` | 旧的收费公开地址。重新上传或填写公开地址后再提交，不要删条目 |
