...
func runExternalURLHealth() (string, error) {
	targets := make([]externalLinkTarget, 0)
	plugins, err := currentSourceStationStore().ListPlugins(sourceItemPublished)
	if err != nil {
		return "", err
	}
	for _, item := range plugins {
		targets = append(targets, externalLinkTarget{
			Kind: sourceKindPlugin, ID: item.ID, Name: item.Name, DeveloperID: item.DeveloperID,
			Location: item.DownloadURL, SHA256: item.SHA256,
		})
	}
	...
	for _, item := range templates {
		targets = append(targets, externalLinkTarget{
			Kind: sourceKindTemplate, ID: item.ID, Name: item.Name, DeveloperID: item.DeveloperID,
			Location: item.TemplateURL, SHA256: item.SHA256,
		})
	}
	...
	for _, target := range targets {
		...
		if location == "" || isStationHostedPackageURL(location) || !strings.HasPrefix(strings.ToLower(location), "https://") {
			skipped++
			continue
		}
		checked++
		if err := verifyExternalPackage(context.Background(), location, target.SHA256); err != nil {
			if delistErr := delistBrokenExternalPackage(target, err.Error()); delistErr != nil {
				failures = append(failures, target.ID+": "+delistErr.Error())
				continue
			}
			delisted++
		}
	...
	if len(failures) > 0 {
		return "", errors.New("外链下架失败：" + strings.Join(failures, "；"))
	}
	return sprintfExternalHealth(checked, skipped, delisted), nil
}

func sprintfExternalHealth(checked, skipped, delisted int) string {
	return "已检查 " + itoaSourceID(int64(checked)) + " 条外链，下架 " + itoaSourceID(int64(delisted)) + " 条；跳过本站托管或非 HTTPS " + itoaSourceID(int64(skipped)) + " 条"
}

func delistBrokenExternalPackage(target externalLinkTarget, reason string) error {
	note := truncateText("外链健康检查失败："+reason, 500)
	var developerID int64
	var name, link string
	if target.Kind == sourceKindTemplate {
		saved, err := currentSourceStationStore().SetTemplateStatus(target.ID, sourceItemHidden, "system-monitor", note)
		if err != nil {
			return err
		}
		developerID = saved.DeveloperID
		name = sourceFirstNonEmpty(saved.Name, target.Name, target.ID)
		link = "/developer-panel/templates"
	} else {
		saved, err := currentSourceStationStore().SetPluginStatus(target.ID, sourceItemHidden, "system-monitor", note)
		if err != nil {
			return err
		}
		developerID = saved.DeveloperID
	...
	persistIndexSnapshot("system-monitor")
	notifyExternalPackageDelisted(developerID, target.Kind, target.ID, name, link, reason)
	return nil
}
