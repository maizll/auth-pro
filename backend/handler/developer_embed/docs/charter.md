...
登记有两种包来源，源站都不执行包内代码。上传 ZIP 时，校验通过后由本站保存文件，并自动填写地址和 SHA256；地址形如 `/api/v1/public/source-packages/<sha256>.zip`。外部 HTTPS 仍可登记，提交审核时检查可达、是不是 ZIP、校验码是否一致。不合规上传返回 `code: 400` 与 `error.field` / `error.rule`，不写库、不留文件。上架之后不再定时复查外链，失效外链不会自动下架。
...
已上架之后，源站不再定期复查外链。外链失效不会自动改为下架（`hidden`），也不会再发「外链失效已下架」。需要从公开目录移除时，由管理员手动下架。
