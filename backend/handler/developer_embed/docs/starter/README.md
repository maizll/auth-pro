...
本包是插件 / 首页模板的登记材料，不是管理端按应用下载的客户端 SDK。源站不执行包内代码。登记时可以上传 ZIP（本站保存并回填地址与 SHA256），也可以只登记外部 HTTPS。外链只在提交审核时核对。上架之后不再定时巡检，也不会因外链失效自动下架。
...
面板：「登记插件」或「登记模板」。包来源可选上传 ZIP 或外部 HTTPS。
