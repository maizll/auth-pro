...
func TestDeveloperSubmitAcceptsStationHostedZipWithoutExternalFetch(t *testing.T) {
	t.Setenv("AUTO_PRO_DATA_DIR", t.TempDir())
	router, _ := sourceStationRouter(t)
	_, dev, _ := sourceApproveDeveloper(t, router, "hosted-dev", "secret")
	fetched := false
	previous := verifyExternalPackage
	verifyExternalPackage = func(context.Context, string, string) error {
		fetched = true
	...
	body := `{"appId":1,"id":"hosted-plugin","name":"托管插件","version":"1.0.0","description":"x","category":"other","downloadUrl":"` + hosted + `","sha256":"` + sha + `"}`
	...
	if fetched {
		t.Fatal("station-hosted package must be checked locally")
	}

	mismatch := `{"appId":1,"id":"bad-hosted","name":"校验不符","version":"1.0.0","description":"x","category":"other","downloadUrl":"` + hosted + `","sha256":"` + sourceTestSHA256() + `"}`
	if rec := sourceJSON(t, router, http.MethodPost, "/api/v1/source/developer/plugins", dev, mismatch); sourceBodyCode(t, rec) != 400 || !strings.Contains(rec.Body.String(), "不一致") {
		t.Fatalf("sha mismatch=%s", rec.Body.String())
	}

	relative := `{"appId":1,"templateKey":"rel-home","name":"相对模板","version":"1.0.0","description":"x","schemaVersion":1,"templateUrl":"templates/demo-home.json","sha256":"` + sourceTestSHA256() + `"}`
	if rec := sourceJSON(t, router, http.MethodPost, "/api/v1/source/developer/templates", dev, relative); sourceBodyCode(t, rec) != 200 {
		t.Fatalf("save relative=%s", rec.Body.String())
	}
	if rec := sourceJSON(t, router, http.MethodPost, "/api/v1/source/developer/templates/rel-home/submit", dev, "{}"); sourceBodyCode(t, rec) != 200 {
		t.Fatalf("submit relative=%s", rec.Body.String())
	}
	if fetched {
		t.Fatal("relative template path must not fetch an external URL")
	}
}
