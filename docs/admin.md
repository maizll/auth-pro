...
| 菜单 | 路径 | 作用 |
| --- | --- | --- |
| 应用管理 | `/license/apps` | 创建应用，获得 `appKey` / `appSecret`。公开校验用这组密钥签名。归档后应用还在，授权和版本保留，只是不能再登记新的目录条目 |
...
| 菜单 | 路径 | 作用 |
| --- | --- | --- |
| 软件目录 | `/source-station/packages` | 按应用查看插件与首页模板。审核、上架、下架、弃用、编辑元数据、版本与回滚 latest。可单条或批量切换绑定应用，不能切到已归档的应用。下拉里的「未归属（应用已删除）」只找回升级前就对不上应用的旧条目。已归档的应用仍在下拉里，并标成已归档 |
