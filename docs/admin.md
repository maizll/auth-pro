...
用户端和代理端没有单独的「绑定域名」或「已绑定站点」菜单。域名绑定在「我的授权」里完成：每条授权显示已绑定域名和「剩余免费更换 X 次」，用「绑定」「更换」「解绑」打开小窗。密钥授权可以在绑定小窗里直接填写新域名，一步完成更换。格式错误、域名已被占用、商业版域名 30 天内再次更换，都会立刻用中文提示。旧地址 `/user/store` 和 `/agent-panel/store` 会转到「我的授权」。购买或开通成功后，会引导去绑定域名。

套餐管理里可以设置「免费更换次数」和「超出后每次更换价格」。次数填 0 表示不能免费更换；留空或 -1 表示不限。价格留空表示免费次数用完后不能再更换。发授权时把这两项记到该授权上，之后改套餐不影响已经发出的授权。已有授权默认不限次数。用户或代理更换已绑定站点、或解绑后再占用名额，才扣次数。首次绑定、名额未满时新增绑定、管理员操作都不扣。次数用完且套餐有价格时，按「更换授权站点」下单，沿用现有支付配置，付款成功后自动完成更换。没有价格时提示「免费更换次数已用完，本套餐不支持付费更换，请联系管理员」。管理员可以在授权列表里加减某条授权的剩余次数。更换、付款和调整都会写入授权日志和审计。
