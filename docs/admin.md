...
顶栏右上角有「升级商业版」。免费版点击后在当前页弹出购买窗口，不打开商店。窗口里有品牌横幅、免费版与商业版对照，以及套餐卡片。已经是商业版时，这里改成金色「商业版」徽章，点击打开授权详情卡。侧栏没有单独的升级菜单。应用商店账号条只显示当前版本，不再放升级按钮。用户端购买授权页不出售商业版套餐。套餐管理里，商业版应用的套餐标有「商业版」。若历史订单只发了普通授权，授权列表和订单列表会提示，并可一键补发。

用户端和代理端没有单独的「绑定域名」或「已绑定站点」菜单。域名绑定在「我的授权」里完成：每条授权显示已绑定域名，用「绑定」「更换」「解绑」打开小窗。格式错误、域名已被占用、商业版域名 30 天内再次更换，都会立刻用中文提示。旧地址 `/user/store` 和 `/agent-panel/store` 会转到「我的授权」。购买或开通成功后，会引导去绑定域名。
