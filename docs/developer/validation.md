...
| `付费条目请上传 ZIP，或填写 HTTPS 外链由本站拉取托管` | 售价大于 0 时上传 ZIP，或改成 `https://` 外链。不要用相对路径 |
| `来源外链必须是 https:// 地址` | 付费外链不要写 `http://` |
| `来源外链地址不合法` | 检查地址能否作为 URL 解析 |
| `外链重定向过多` | 减少跳转，最多 3 次 |
| `外链重定向离开 https` | 重定向后的地址仍须是 HTTPS |
| `外链 ZIP 超过 20 MiB` | 缩小压缩包 |
| `外链返回状态码 …` / `外链不可达` | 确认地址能公开下载；失败时不会保存，重新拉取失败时保留旧包 |
| `这条没有可重新拉取的来源外链` | 只有曾经用 HTTPS 外链拉取托管的付费条目才能重新拉取 |
| `年付尚未开放，当前只支持买断` | 去掉 `billing: "yearly"` |
| `付费条目暂不能上架，购买与交付将在后续版本开放` | 先保持免费，或只保存草稿 |
| `改为收费前请选择：已下载过的老用户继续免费，或所有人都需购买` | 已公开的免费条目可以改为收费。保存前在弹窗里选择老用户继续免费，或所有人都需购买 |
