...
登记有两种包来源，源站都不执行包内代码。上传 ZIP 时，校验通过后由本站保存文件，并自动填写地址和 SHA256；地址形如 `/api/v1/public/source-packages/<sha256>.zip`。外部 HTTPS 仍可登记，提交审核时检查可达、是不是 ZIP、校验码是否一致。不合规上传返回 `code: 400` 与 `error.field` / `error.rule`，不写库、不留文件。本站托管地址在提交时只核对本地文件，不向外网发请求。
...
- 提交审核时，外链会由服务端下载：不可达返回 `外链不可达`，不是 ZIP 返回 `外链不是 ZIP`，校验码不符返回 `外链内容与 sha256 不一致`。解析后的地址必须是公网单播；回环、私网、链路本地、运营商级 NAT 和云元数据返回 `拒绝访问非公网地址` 或 `拒绝访问链路本地或云元数据地址`，并且不会读取该目标的响应体。重定向离开 https 返回 `外链重定向离开 https`。本站托管地址只核对本地文件。
...
外链在**提交审核**时由服务端下载并核对 sha256，且只接受公网 HTTPS。草稿保存不访问外链。安装侧仍会再验一次：`软件源模板 ZIP 的 SHA256 校验失败`。插件安装若包内 `id` 不一致：`plugin.json 的插件 ID 必须与软件源一致`。从软件源安装插件时，远程 HTTP(S) 包必须带清单 `sha256`，不一致则不安装。本站托管地址提交时只核对本地文件。
