...
仓库根目录 `VERSION` 是产品线版本信源（当前 `1.4.4`）。发布时用 tag / `-ldflags` 注入 `auto_pro/config.AppVersion` 与前端 `VITE_VERSION`；未注入时仓库默认也是 `1.4.x`，不会静默显示 `1.0.0`。
