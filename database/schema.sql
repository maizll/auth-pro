...
CREATE TABLE `apps` (
  ...
  `enabled`     TINYINT(1)   DEFAULT 1 COMMENT '是否启用: 1启用 0禁用',
  `commercial_product` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否作为本站商业版出售，全站最多一个',
...
CREATE TABLE `licenses` (
  ...
  `source`           ENUM('admin','agent','user_purchase','card','store_bind','store_purchase') NOT NULL COMMENT '来源: admin后台开通/agent代理开码/user_purchase用户购买/card卡密兑换/store_bind商店绑定/store_purchase商店购买',
