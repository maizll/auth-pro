...
## [源站] 2026-09-20 — 弃用后从软件源目录清除

- 弃用插件/模板后，公开 `index.json` 不再保留该 id（与下架相同）；库内可保留 `deprecated` 历史行。
- 软件目录默认列表与开发者工作台不再把弃用项当成在架商品；状态筛选「已弃用」仍可查历史。
- 弃用当前 latest 且没有其它已发布版本时，条目退出公开目录。
- 回归：deprecate → 公开 index 不含 id。
