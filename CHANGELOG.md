...
## [v1.6.1] 2026-09-26 — 软件目录外链校验，收费条目改走私有 GitHub

- 后台「上传安装包」来源三选一：上传压缩包、公开地址（仅免费）、私有 GitHub 仓库（收费推荐）。都不必填。按钮不可用时，弹窗里用中文写明还差什么。
- 免费公开地址：本站下载并校验清单、越界路径和大小，自动填写 sha256，不保存压缩包。重定向每一跳都拒绝内网地址。目录里保存的是填写的地址。
- 收费私有仓库：GitHub 只读令牌（fine-grained，Contents 只读）加密存放，页面只显示已配置或未配置。管理员的令牌在软件源设置，只用于管理员登记的条目。开发者的令牌在开发者面板按账号分别保存，只用于该开发者自己的插件和模板。登记时粘贴 Release 资产链接，本站用对应令牌核对一次后删除临时文件，不长期存 ZIP。买家付款后，本站用条目所属账号的令牌向 GitHub 换取几分钟有效的临时下载地址并交给买家，不转发文件流量。买家看不到仓库地址和令牌。客户端仍用 sha256 核对。
- 开发者面板登记插件和模板与后台相同：来源三选一，按钮不可用时用中文说明原因，旧的收费公开地址显示「需要改成私有仓库来源或上传 zip 才能继续收费出售」。
- 令牌失效或找不到安装包时，用中文报错。定时检查把该条目标成不可用并通知管理员，不自动下架。
- 上传压缩包仍可作为收费条目的本站托管方式。
- 以前因为收费条目不能挂外部地址而被标成隐藏或弃用的目录行，升级后回到草稿并注明「需要改成私有仓库来源或上传 zip 才能继续收费出售」。不改已上架、免费、以及已经是本站托管或私有仓库的行。
