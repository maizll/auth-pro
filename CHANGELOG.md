...
## [v1.6.1] 2026-09-26 — 软件目录只填外部地址也能校验安装包

- 后台「上传安装包」里，压缩包和外部地址不再同时标成必填。只填合法 https 外部地址时，「仅校验解析」和「校验并保存元数据」可以点击。本站下载该 ZIP，按上传同样的规则校验清单、越界路径和大小，并自动填写 sha256。按钮仍不可用时，弹窗里用中文写明还差什么。
- 下载会跟随重定向。每一跳都拒绝内网、回环、链路本地和云元数据地址。保存的是填写的地址，不是跳转后的临时地址。
- 售价为 0 时，目录继续使用这条外部地址。售价大于 0 时，本站拉取后写入私有目录，买家看不到外链。
- 开发者登记和「登记外部地址」本来就可以只填网址。上传首页模板是安装本站页面，没有外部地址，仍然必须选择 ZIP。
