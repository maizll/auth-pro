...
- 收费私有仓库：在软件源设置保存一枚 GitHub 只读令牌（fine-grained，Contents 只读），加密存放，页面只显示已配置或未配置。登记时粘贴 Release 资产链接，本站用令牌核对一次后删除临时文件，不长期存 ZIP。买家付款后，本站向 GitHub 换取几分钟有效的临时下载地址并交给买家，不转发文件流量。买家看不到仓库地址和令牌。客户端仍用 sha256 核对。
