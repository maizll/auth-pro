...
- 接口报错时红色提示只弹一次。拦截器弹出后，页面 catch 不再重复弹同一条；同一文案在短时间内的第二次也会被丢掉。商业版 402 仍只走升级提示。
- 源站保存产品应用标识时会去掉首尾空格，并立刻检查该 app_key 在应用表里存在且已启用。找不到时提示「找不到该应用标识，请从 应用管理 复制 app_key」。设置页改为从已启用应用中下拉选择。买家绑定查询同样会去掉首尾空格。
- 「商店预留」里的免费套餐改为下拉，只列出所选产品应用在套餐管理中的套餐，可以留空。保存时核对套餐存在且属于该应用，否则提示「该套餐不属于所选产品应用，请到 套餐管理 选择」。卡片说明写明各字段的用途和去哪里找。
...
- 只允许 HTTPS，限制大小、超时和重定向次数。拒绝内网、回环、链路本地和云元数据地址，包括重定向之后的地址和 DNS 解析结果。
- 来源外链只出现在开发者和源站管理员界面。公开目录、商店和买家下载链接都不包含它。买家安装仍走授权核对和 10 分钟签名下载。
- 开发者和管理员可以重新拉取。成功后替换托管包并更新 sha256 与版本；失败保留旧包。定时检查来源外链是否还能访问，失效只提示「来源外链不可用，重新拉取将失败」，不把已托管的付费条目下架。免费外链仍在提交审核时校验。
- 根目录 `VERSION` / `AppVersion` / `VITE_VERSION` 默认 `1.5.8`。发布说明见 `docs/release-notes-1.5.8.txt`。

## [v1.5.7] 2026-09-26 — 在线更新交给进程守护拉起

- 后台在线更新在宝塔「进程守护管理器」（supervisor 自动重启）下可以完成。当前进程由守护或 systemd 拉起时，不再调用守护的停止命令，也不再 `nohup`。先替换程序和前端，写待验证标记，然后只退出本进程，由守护按 `backend/start.sh` 拉起新版本。
- 新版本连续健康检查失败时，`start.sh` 在同一次启动里还原备份并执行旧程序，更新页显示中文失败原因。检查不把守护打成 FATAL。没有守护时仍先确认端口空闲再自行拉起。其它站点的 `auth_pro` 不会被结束。
- 1.5.5/1.5.6 的在线更新停不住宝塔自己的 supervisor，杀进程后旧版本会被立刻拉起，所以那一次升级不能用后台在线更新。请用 1.5.7 包里的 `baota-upgrade.sh --start`。装上 1.5.7 之后的下一次在线更新才走新逻辑。步骤见 `docs/release-notes-1.5.7.txt` 和 `docs/deployment.md`。
- 根目录 `VERSION` / `AppVersion` / `VITE_VERSION` 默认 `1.5.7`。发布说明见 `docs/release-notes-1.5.7.txt`。

## [v1.5.6] 2026-09-26 — 内置商店签名公钥

- 发行包内置源站商店签名公钥。源站升级到 1.5.6 后可以签发授权快照，买方可以验签。商业版购买与授权下发可以正常使用。
- 源站须先升级到 1.5.6，并保留已有的 `store/snapshot-ed25519.key`。不要重新执行 `store-keygen`。私钥与内置公钥不一致时仍拒绝签发。构建时仍可用 `AUTH_PRO_STORE_SNAPSHOT_PUBLIC_KEY` 或 ldflags 覆盖公钥。
- 根目录 `VERSION` / `AppVersion` / `VITE_VERSION` 默认 `1.5.6`。发布说明见 `docs/release-notes-1.5.6.txt`。

## [v1.5.5] 2026-09-26 — 在线更新先停后启、失败回滚与后端不可达页

- 在线更新、宝塔升级和覆盖安装都先停旧进程并确认端口空闲，再启动新进程。本站 `auth_pro` 孤儿（包括父进程为 1、不响应的进程）会先 SIGTERM，超时后 SIGKILL；其它程序或其它站点的 `auth_pro` 不会被结束。健康检查失败则回滚。端口被占时后端日志会写明占用者和处理办法。更新页重启轮询超过 3 分钟会显示失败原因和恢复步骤。
- 发布包增加 `backend-unavailable.html`。Nginx 对 502/503/504 返回该页；宝塔安装/升级脚本会幂等写入站点配置。已打开的前端在接口连续网络错误或 502/504 时跳到同一页。
- 从 1.5.3 或 1.5.4 在线更新到 1.5.5 这一次仍由旧版更新逻辑执行，可能卡住。恢复步骤见 `docs/deployment.md`。
- 根目录 `VERSION` / `AppVersion` / `VITE_VERSION` 默认 `1.5.5`。发布说明见 `docs/release-notes-1.5.5.txt`。
