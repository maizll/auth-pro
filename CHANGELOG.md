...
- 移除管理端「定时任务 / 系统监控」：页面、菜单、接口和进程内调度一并删除。已上架外链不再自动下架。版本号仍为 1.5.0，留待后续发布。
- 移除开发者文档页上的插件卡片预览和首页模板实时预览（含不提交密码的预览登录）。章程、入门包和 Skill 下载保留。

## [v1.5.0] 2026-09-23 — ZIP 上传、外链巡检与系统监控

- 开发者登记可以上传 ZIP：源站保存文件并回填地址与 SHA256；外部 HTTPS 仍然保留，提交审核时检查可达、ZIP 与校验码（#66）。
- 定时任务只巡检外链。失败则从软件源目录下架并通知开发者。本站托管的上传不参与巡检。
- 管理端新增「定时任务 / 系统监控」，进程内执行外链健康、授权校验、支付回调路由、在线更新源和磁盘用量五项任务（#66）。
- 开发者 Skill 与入门包 ZIP 编译进二进制，不再依赖进程工作目录。开发文档页用同一份示例渲染正式首页（`cartoon-blue` / `fintech-gold`），预览登录只打开宿主登录框、不提交密码（#65）。
