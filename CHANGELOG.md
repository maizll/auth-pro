...
## [v1.6.3] 2026-09-26 — 软件源按 JSON 目录处理，已删除应用的地址可以转走

- 添加或刷新软件源时，以 `.json` 结尾或返回 JSON 的地址按 JSON 目录处理，不再把本站 `index.json` 当成 Git 仓库克隆。类型和地址不一致时当场改成正确类型并提示。已经记成 Git、地址却以 `.json` 结尾的旧记录会在启动时改回 JSON，重复执行不会再改。Git 克隆发现不是仓库时，提示「该地址不是 Git 仓库，可能是 JSON 目录，请修改源类型」，不再显示 git 的原始报错。
- 应用删除或归档后，原来的 `/software-source/<旧标识>/index.json` 不再被当成空目录或 Git 仓库。没有把地址转走时，接口返回 HTTP 410 的 JSON，刷新方显示「该软件源对应的应用已删除或归档」，后台可以「恢复应用」或「更换源地址」。归档时可以勾选把该应用的软件源地址转到另一个应用，旧地址继续返回目标应用的目录。1.6.1 之前已经真删的应用，可以在源站设置里把旧标识映射到现有应用，重复保存只更新目标。恢复应用后，这条地址重新返回它自己的目录。
- 程序内置的默认软件源改为 `https://auth.maizll.com/software-source/app_f93896d80066_5811/index.json`，类型是 JSON 目录。升级时把旧默认地址 `https://auth.maizll.com/software-source/app_4e85b4724223_2603/index.json` 换成这个地址，并把类型改成 JSON。再执行一次不会重复添加，自己添加的其它软件源保持不动。还没有任何软件源的安装会写入这一条。
- 根目录 `VERSION` / `AppVersion` / `VITE_VERSION` 默认 `1.6.3`。发布说明见 `docs/release-notes-1.6.3.txt`。商店公钥仍是 `pwAizm/sOyWCu+qi8+Dl/xJr0Upuamh5u7vL3wGT14A=`。

## [v1.6.2] 2026-09-26 — 顶栏升级商业版，站点更换次数，免费目录改收费

- 已公开的免费软件目录条目可以在编辑弹窗里改为收费，也可以改回免费。保存前说明公开下载地址会作废，并选择「已下载过的老用户继续免费使用」（默认）或「所有人都需购买」。选老用户继续免费时，按已有下载记录给对应授权或账号发放该条目的免费权益，重复保存不会重复发放。选所有人都需购买时不发放，并收回之前这类免费权益，商业版也不再直接包含这一条。改为收费后安装包迁入已配置的收费仓库，未配置时由本站私有托管；外链会自动拉取后再托管。下载走付费校验。两种切换都写入审计。开发者条目同样适用，但仍不能上架，保存后会从公开目录下架。
- 套餐可以限制免费更换授权站点的次数，并设置超出后每次更换的价格。0 次表示不能免费更换，不填或 -1 表示不限；价格不填表示用完后不能付费更换。发授权时把这两项记到授权上，旧授权默认不限。用户和代理在「我的授权」里看到剩余次数，更换或解绑已绑定站点才扣次数，管理员不扣。次数用完后按「更换授权站点」付款，成功后自动更换；本套餐不支持付费更换时提示联系管理员。管理员可手动加减剩余次数。更换、付款和调整都写入授权日志和审计。
- 用户端和代理端去掉单独的「已绑定站点」页。域名绑定、更换、解绑都在「我的授权」里用小窗完成，沿用原来的格式校验、占用检查和商业版 30 天更换限制。旧地址会转到「我的授权」。购买成功后引导绑定域名。
- 标记为「作为本站商业版出售」的应用，套餐不再出现在用户端和代理端的授权购买页。购买接口会拒绝这类套餐，并提示到管理后台顶栏购买。顶栏付款、以及已经付过款的授权订单，都走同一条商业版开通：写入商业版权益，买家站点同步签名快照后解除单应用限制。
- 授权列表和订单列表会标出「在授权购买页买了商业版套餐但没开通」的已支付订单，可一键补发。重复点击不会重复发放。套餐管理里，这类套餐显示「商业版」。

- 管理后台顶栏右上角增加带图标的「升级商业版」。点击后在当前页弹出购买窗口，不跳转商店。窗口里说明免费版只能创建 1 个授权应用、商业版解除限制，并从源站拉取套餐。付款沿用原来的商业版购买接口，窗口内显示支付状态。付款成功后提示「已升级为商业版」，关闭后顶栏和限制提示立即更新。源站未配置、网络不通、支付失败或超时都会给出中文原因。
- 已经是商业版时，顶栏改为金色「商业版」徽章。点击打开授权详情卡，显示到期时间、绑定账号、授权域名和授权编号，不再显示升级。
- 购买窗口改为品牌横幅、免费版与商业版对照（授权应用数量、官方付费插件、官方付费首页模板），以及套餐卡片。付款区显示二维码、剩余时间和安全支付说明。支付成功后有轻量庆祝动画，并提示「已升级为商业版」。对照文案集中在前端常量里，只写代码里真实存在的差别。
- 应用商店账号条、应用管理工具栏里单独的升级、续费、查看授权按钮已去掉。免费版的限制提示（徽章、提示条、说明框）保留，其中的「升级」打开同一个购买窗口。手机端顶栏也能点开。
- 根目录 `VERSION` / `AppVersion` / `VITE_VERSION` 默认 `1.6.2`。发布说明见 `docs/release-notes-1.6.2.txt`。商店公钥仍是 `pwAizm/sOyWCu+qi8+Dl/xJr0Upuamh5u7vL3wGT14A=`。

## [v1.6.1] 2026-09-26 — 软件目录外链校验，收费包统一放进站长的私有仓库

- 后台和开发者端登记收费条目时，只需上传 ZIP 或填写公开地址。存储位置对开发者透明。
- 免费公开地址：本站下载并校验清单、越界路径和大小，自动填写 sha256，不保存压缩包。重定向每一跳都拒绝内网地址。目录里保存的是填写的地址。
- 收费包：站长在软件源设置填写同一个私有 GitHub 仓库，以及一枚 Contents 读写令牌（加密保存，页面不回显明文）。校验通过后，本站用这枚令牌把 ZIP 上传到该仓库的 Release，标签和文件名按条目标识加版本区分，然后删除本站临时文件。买家付款后，本站核对授权，向 GitHub 换取几分钟有效的临时下载地址，不转发文件流量。买家看不到仓库地址和令牌。客户端仍用 sha256 核对。
- 尚未配置收费仓库时，收费包暂存在本站，后台用中文提醒去软件源设置填写仓库和令牌。开发者端不显示这句提醒。
- 令牌失效或找不到安装包时，用中文报错。定时检查把该条目标成不可用并通知管理员，不自动下架。
- 以前因为收费条目不能挂外部地址而被标成隐藏或弃用的目录行，升级后回到草稿并注明「需要重新上传 zip 或填写公开地址，才能继续收费出售」。不改已上架、免费、以及已经是本站托管或收费仓库的行。
- 软件目录可以切换条目绑定的应用，支持多选。版本、价格和安装包跟着走，并写入审计。目标应用里已有相同标识时用中文拒绝，也不能切到已归档的应用。应用下拉增加「未归属（应用已删除）」，只用来找回升级前就已经对不上应用的旧条目。删除应用改为归档：应用行还在，授权和版本都保留。下面还有目录条目时，可以迁到另一个应用再归档，也可以直接归档并继续挂在原应用上。开发者只能在自己能登记的未归档应用之间切换自己的条目。
- 收费安装包走统一存储驱动。1.6.1 只有「本站暂存」和「站长的 GitHub 私有仓库」。每个版本记下存在哪种存储、对象键和校验码。仓库已配置但上传失败时不写目录、也不改存本站。设计说明见 `docs/design/catalog-storage-ownership.md`。
- 误点弃用后，管理员可以把插件或模板恢复为草稿，不会自动上架，需要再审核才能出现在公开软件源。已归档的应用可以恢复，授权、版本和目录绑定都还在。授权不是启用状态时，不能再下载收费包；临时禁用不会清掉已购权益，重新启用后可以再下。本站暂存的收费包文件不见了，会标成不可用并通知管理员，不会自动下架。其余生命周期缺口记在设计文档第 7 节，放到 1.6.2 和 1.6.3。
- 支付宝当面付预下单把 `charset=utf-8` 放进网关地址，请求按 UTF-8 签名。网关用 GBK 返回的错误会按响应字符集转成中文；验签失败、AppID 不正确、没有当面付权限会写成明确提示。
- 根目录 `VERSION` / `AppVersion` / `VITE_VERSION` 默认 `1.6.1`。发布说明见 `docs/release-notes-1.6.1.txt`。

## [v1.6.0] 2026-09-26 — 修复编辑应用保存误报「应用不存在」

- 应用管理里编辑已有应用后点保存，如果名称、备注、启用状态和授权方式都没变，会误报「应用不存在」。只打开「作为本站商业版出售」时也会中招，而且开关没有写进数据库。原因是 MariaDB 在赋值和原值相同时把影响行数报成 0，后端把 0 当成应用被删掉，商业版字段还没写就返回了。
- 现在先确认这条应用还在，再继续保存商业版开关。真正不存在的应用仍然报「应用不存在」。
- 套餐管理、授权列表、支付配置的保存不按「影响行数为 0」判断记录消失，没有这个误报。应用管理编辑保存成功后会关闭对话框并刷新列表，这条路径没有主键问题。
- 在线更新触发后改为轮询 `/api/system/version`（带防缓存参数）。重启期间的 502/504、断网和 nginx 返回的 HTML 错误页都当作「重启中」继续等。版本等于目标版本时提示「更新完成，正在刷新」并自动刷新页面。旧版本且后端报告已回滚时显示中文原因。整体最长等 10 分钟，重启阶段最长 3 分钟，超时后停止转圈。`index.html` 增加 no-cache，避免刷新后仍用旧入口页。线上 1.5.9 的页面是旧代码，从 1.5.9 在线更新到 1.6.0 这一次仍可能要手动刷新；从 1.6.0 再往后更新才会自动刷新。
- 根目录 `VERSION` / `AppVersion` / `VITE_VERSION` 默认 `1.6.0`。发布说明见 `docs/release-notes-1.6.0.txt`。

## [v1.5.9] 2026-09-26 — 商业版并入原有授权系统

- 商业版不再单独开页面。在应用的新增/编辑弹窗打开「作为本站商业版出售」（全站最多一个，换应用时会提示并切换）。价格用该应用在套餐管理里已启用且有价格的套餐，时长 0 为永久。授权仍在授权列表，来源显示「商店绑定」「商店购买」并可筛选，行内可授予或吊销商业版。商店订单并入订单列表，类型为「商业版」「付费插件」「付费模板」，并显示已支付合计。收款仍用支付配置。
- 删掉源站运营下的商业版设置、商店订单、主授权与权益、商业版收入，以及源站设置里的「商店预留」卡片。升级时会删掉旧菜单。旧地址分别转到套餐管理、订单列表和授权列表。
- 应用列表对出售中的应用显示「商业版产品」。缺少有价格的套餐、未配支付或签名密钥未生成时，点状态即可去处理；密钥可在这一行生成。
- 旧的手填产品应用标识如果能对上某个应用（去掉首尾空格后比较），升级后自动标为出售。买家绑定查询仍会去掉首尾空格。免费主授权不再手填套餐，绑定后自动建立且不挂套餐。
- 菜单中文与 `locales/langs/zh.json` 共用一份来源。映射表里没有的 `menus.*` 标题回退为「未命名菜单」，不再显示英文 key 片段。前端单测和 `go test` 会检查前端路由、`menu_spec.go`、`menu_seed.sql` 里的 `menus.*` 标题是否都有中文。
- 内嵌页的页签标题由 iframe 改为「内嵌页面」。
